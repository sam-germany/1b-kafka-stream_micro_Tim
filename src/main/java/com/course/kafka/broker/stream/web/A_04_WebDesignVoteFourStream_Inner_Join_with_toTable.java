...
// video  117
//@Configuration
public class A_04_WebDesignVoteFourStream_Inner_Join_with_toTable {
...
//color
       var colorTable = builder.stream("t.commodity.web.vote-color", Consumed.with(stringSerde, colorSerde, new WebColorVoteTimestampExtractor22(), null))
                                                 .mapValues( v -> v.getColor()).toTable();

//layout
       var layoutTable = builder.stream("t.commodity.web.vote-layout", Consumed.with(stringSerde, layoutSerde, new WebLayoutVoteTimestampExtractor22(), null))
                                                  .mapValues( v -> v.getLayout()).toTable();


// join color-Table  + layout-Table
var joinTable22 = colorTable.join(layoutTable, this::voteJoiner22, Materialized.with(stringSerde, designSerde));
joinTable22.toStream().to("t.commodity.web.vote-four-result");
...
here we are creating a Stream and the key is alread sended as "username" and here we are setting the value to the key
...
to the key
(3)
var joinTable22 =  = colorTable.join(layoutTable, this::voteJoiner22)       <--  little bit trickey, as when we are
...
--------------------------------------------------------------------------------------------------------------------------
Rule: Main point is both Left and Right side are mandatory so when both are filled with value a new record is created
